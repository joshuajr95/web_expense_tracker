...
/*
 * Finds whether the given DOM element is a descendent of the
 * ancestor element.
 */
...
function isDescendent(domElement, ancestorElement)
{
    ...
    let domRoot = document.getRootNode();
...
function displayForm()
{
    ...
    nodes.forEach( element => {
        if(!isDescendent(element, input_form))
        {
...
function hideForm()
{
    ...
    input_form.reset();
    editing = false;
    input_form_div.style.display = "none";
}
...
document.querySelector('#form-exit-button').addEventListener('click', hideForm);